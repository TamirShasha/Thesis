...
class LengthEstimator2DWellSeparated:
    ...
    def _calc_length_likelihood(self, signal_filter, expected_num_of_occurrences):
        ...
        # mapping = utils.dynamic_programming_2d_well_separated(n, expected_num_of_occurrences, d, sum_yx_minus_x_squared)
        # well_separated_result = mapping[0, 0, expected_num_of_occurrences]

        # Computing remaining parts of log-likelihood
        log_pd1 = -utils.dynamic_programming_2d_well_separated(n, expected_num_of_occurrences, d,
                                                               np.zeros(sum_yx_minus_x_squared.shape))[0, 0, expected_num_of_occurrences]
        log_pd2 = - (np.log(2) + utils.log_size_S_2d_1axis(n, expected_num_of_occurrences, d))
        print(log_pd1, log_pd2)
        log_prob_all_noise = self.log_prob_all_noise

        likelihood = log_pd + log_prob_all_noise + well_separated_result
        return likelihood
